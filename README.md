...
该入口默认读取项目目录下的 `Vocabulary.xlsx`，由本地 Node 服务直接解析 `.xlsx` 后启动 `http://127.0.0.1:3765/`。
页面中可点击 `选择xlsx文件` 使用自定义 Excel；选择后的文件会缓存到 `.word-memory-cache`，下次启动继续使用。

## Excel文件要求

仅支持 `.xlsx` 文件，支持以下两种格式。

两工作表格式：

- `单词总表`，表头依次为：`单元`、`单词`、`词性和中文意思`
- `八年级上下册英语全单词汇总`，表头依次为：`单词`、`音标`、`词性`、`中文意思`

单词按 `单词` 列英文文本去重合并。两张表同时存在同一英文单词时，展示信息优先使用 `八年级上下册英语全单词汇总` 的音标、词性、中文意思。

单工作表格式：

- 表头包含：`Day`、`英文单词`、`中文释义`
- `Day` 作为单元，`英文单词` 作为英文，`中文释义` 作为中文；若中文释义里包含 `n.`、`v.`、`adj.` 等词性标记，会自动拆到词性列。
