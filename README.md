...
- `单词总表`，表头依次为：`Day`、`序号`、`单词`、`中文意思`
- `八年级上下册英语全单词汇总`，表头依次为：`单元`、`序号`、`单词`、`音标`、`词性`、`中文意思`、`Page`、`总表里有`

单词按两张表的 `单词` 列英文文本去重合并。两张表同时存在同一英文单词时，展示信息优先使用 `八年级上下册英语全单词汇总` 的音标、词性、中文意思。`Page` 和 `总表里有` 两列不参与解析。
